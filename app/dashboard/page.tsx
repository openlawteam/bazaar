...
import { userFeedPayload, userListingsPayload, userWantsPayload } from "@/lib/bazaar";
...
export default async function DashboardPage() {
  ...
  const [{ wants }, { listings }, { feed }] = await Promise.all([
    Promise.resolve(userWantsPayload(user.id)),
    Promise.resolve(userListingsPayload(user.id)),
    userFeedPayload(),
  ]);
      ...
      <Tabs defaultValue={intent} className="w-full">
        ...
        <TabsContent value="buy" className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="space-y-3">
            ...
            {feed.slice(0, 4).map((listing) => (
              <Card key={listing.id} className="gap-0 overflow-hidden bg-white py-0">
                <Link href={`/listings/${listing.id}`} className="block">
                  <ListingImage
                    id={listing.id}
                    imageUrl={listing.imageUrl ?? "/demo/real-chair.jpg"}
                    title={listing.title}
                    className="h-40 border-b-2"
                  />
                </Link>
                <CardHeader>
        ...
        <TabsContent value="sell" className="grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <section className="grid gap-3">
            <div className="flex flex-col gap-3 rounded-[1.5rem] border-2 border-black bg-white p-4 shadow-[5px_5px_0_#111] sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-2xl font-black uppercase tracking-tight">Your for-sale listings</h2>
                <p className="text-sm font-semibold text-black/65">
                  Browse every listing with photos, price, and pickup details.
                </p>
              </div>
            ...
            {listings.length > 0 ? (
              listings.map((listing) => (
                <Card key={listing.id} className="gap-0 overflow-hidden bg-white py-0">
                  <div className="grid min-h-52 gap-0 md:grid-cols-[240px_minmax(0,1fr)]">
                    <ListingImage
                      id={listing.id}
                      imageUrl={listing.imageUrl}
                      title={listing.title}
                      className="h-56 border-b-2 md:h-full md:border-b-0 md:border-r-2"
                    />
                    <div className="py-6">
                      <CardHeader>
